...
## Disables all input, but not physics interactions
...
@export var TOGGLE_LIGHT : String = "toggle_light"
...
func check_controls(): # If you add a control, you might want to add a check for it here.
	...
	if !InputMap.has_action(TOGGLE_LIGHT):
		push_error("No control mapped for flashlight. Please add an input map control. Disabling sprinting.")
...
func _process(delta):
	...
	HAND.rotation.x = lerp(HAND.rotation.x, HEAD.rotation.x, 5 * delta)
	HAND.rotation.y = lerp(HAND.rotation.y, HEAD.rotation.y, 5 * delta)
